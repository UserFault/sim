...
namespace Mary.DatabaseAdapters
{
    ...
    public class MDbAdapterBase
    {
        ...
        /// Эта функция должна быть переопределена в производном классе.
        /// </summary>
        ...
        /// <summary>
        /// RT-Создать объект класса БД в зависимости от типа БД в проекте
        /// </summary>
        ...
        /// </remarks>
        /// <seealso cref=""/>
        public static MDbAdapterBase LoadDbAdapter(MEngine engine, MDatabaseType databaseType)
        {
            //создать массив аргументов для конструктора объекта адаптера
            Object[] args = new Object[] { engine };
            
            switch (databaseType)
            {
                case MDatabaseType.NoDatabase:
                    return new MDbAdapterNoDb(engine);//TODO: заменить на нужный класс
                //break;
                case MDatabaseType.MicrosoftSqlServer2005:
                    //return new MDbAdapterMsSql2005(engine);//заменить на нужный класс
                    return loadDbAdapterFromAssembly("MsSql2005DbAdapter", args);
                //break;
                case MDatabaseType.MsAccess:
                    //return new MDbAdapterMsJet(engine);//заменить на нужный класс
                    return loadDbAdapterFromAssembly("MsJetDbAdapter", args);
                case MDatabaseType.MySql5:
                    return loadDbAdapterFromAssembly("MySql5DbAdapter", args);
                case MDatabaseType.Sqlite3:
                    return loadDbAdapterFromAssembly("Sqlite3DbAdapter", args);
                default:
        ...
        /// <summary>
        /// RT-Загрузить сборку с указанным адаптером БД
        /// </summary>
        /// <param name="assemblyAndClassName">Название класса адаптера и сборки, содержащей этот адаптер.</param>
        /// <param name="args">Массив аргументов конструктора объекта адаптера БД.</param>
        /// <returns></returns>
        private static MDbAdapterBase loadDbAdapterFromAssembly(string assemblyAndClassName,  Object[] args)
        {
            //сборки адаптеров должны располагаться в каталоге со сборкой движка
            //1 получить каталог с текущей сборкой
            Assembly maryAssembly = System.Reflection.Assembly.GetExecutingAssembly();
            String asmPath = maryAssembly.Location;
            asmPath = Mary.MUtility.getPathFromUNCpath(asmPath);
            asmPath = Path.GetDirectoryName(asmPath);
            //2 собрать путь к загружаемой сборке
            asmPath = Path.ChangeExtension(Path.Combine(asmPath, assemblyAndClassName), ".dll");
            //3 загрузить сборку в домен приложения. А, нет - в тот домен, куда сборка движка загружена.
            //  Это может оказаться не основной домен приложения!
            //AppDomain domain = Thread.GetDomain();
            //не буду проверять перебором - все равно других доменов и нет и взять неоткуда.
            //foreach (Assembly a in domain.GetAssemblies())
            //{
            //    if(a.
            //}
            Assembly aa = Assembly.LoadFile(asmPath);
            //4 если что-то пошло не так, выбросить исключение с понятным описанием.
            //5 найти в загруженной сборке класс БД по его className и/или атрибуту класса.
            //6 загрузить класс и вернуть его объект
            String typename = String.Format("{0}.{0}", assemblyAndClassName);//имя загружаемого класса должно включать в себя имя сборки, иначе никак.
            MDbAdapterBase result = (MDbAdapterBase) aa.CreateInstance(typename, true, BindingFlags.CreateInstance | BindingFlags.Instance | BindingFlags.Public, null, args, null, null);

            if (result == null)
                throw new Exception("Не удалось загрузить выбранный адаптер БД");
            return result;
        }
