...
#region Properties
        ...
        public override DateTime CreaTime
        {
            ...
            //__setCreaTime(DateTime val) вместо set
        }
        ...
        public override DateTime ModiTime
        {
            ...
            //__setModiTime(DateTime val) вместо set
        }
        ...
        public override MCellMode CellMode
        {
            ...
            //__setCellMode(MCellMode val) - вместо set
        }
        ...
        #region Служебные функции-асессоры
        //Эти функции были раньше в проперти internal set, но теперь они должны быть публичными, но служебными.
        //Поэтому их надо спрятать от документации, чтобы пользователь их не использовал. Если это возможно.
        //TODO: если эти функции не используются, удалить их в конце релиза, и их родителей и наследников тоже.
        /// <summary>
        /// NT-Служебная внутренняя функция
        /// Cell (saving) mode: Compact, Normal, DelaySave, Temporary
        /// </summary>
        /// <param name="val"></param>
        public override void __setCellMode(MCellMode val)
        {
            if (val == MCellMode.Compact) 
                throw new Exception("Try to set Compact cell mode for MCellB cell");
            //Check Temporary state restriction
            if (((m_cellMode == MCellMode.Temporary) && (val != MCellMode.Temporary)) || ((val == MCellMode.Temporary) && (m_cellMode != MCellMode.Temporary)))
            {
        ...
        /// </summary>
        /// <param name="val"></param>
        public override void __setCreaTime(DateTime val)
        {
            m_creatime = val; m_moditime = DateTime.Now;
            //save changes
            if (m_cellMode == MCellMode.Normal)
                MCell.Container.DataLayer.setCellCreationTime(m_cellid, val);
        }
        /// <summary>
        /// NT-Служебная внутренняя функция
        /// Last modification timestamp
        /// </summary>
        /// <param name="val"></param>
        public override void __setModiTime(DateTime val)
        {
            m_moditime = val;
            //не сохраняется в БД, чтобы избежать кольцевой зависимости
        }
        #endregion
