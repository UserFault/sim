...
namespace MsJetDbAdapter
{
    
    /// <summary>
    /// NR-Класс адаптера БД для БД MS Access 2003
    /// </summary>
    /// <remarks>
    /// Вся сборка должна быть скомпилирована для архитектуры x86, иначе БД не запустится на Windows 7 и выше.
    /// </remarks>
    public class MsJetDbAdapter: MDbAdapterBase
    {
        /// <summary>
...
#region *** Fields ***
        ...
        /// </summary>
        public MsJetDbAdapter(MEngine engine): base(engine)
        {
            //throw new Exception("Привет из MsJetDbAdapter");
            return;//add some inits here
        }
        /// <summary>
        /// Destructor
        /// </summary>
...
#region *** Функции создания и изменения БД *** - одна не реализована
        ...
        public override void DatabaseCreate(MSolutionInfo info)
        {
            ...
            String DbFilePath = Path.Combine(info.getCurrentSolutionDirectory(), MsJetDbAdapter.DatabaseFileName);
            ...
            MDbAdapterBase.extractDbFile(DbFilePath, Properties.Resources.OleDbBaseFile);
        ...
        public override void DatabaseClear()
        {
            ...
            this.ClearTable(MsJetDbAdapter.CellTableName);
            this.ClearTable(MsJetDbAdapter.LinkTableName);
            this.ClearTable(MsJetDbAdapter.ContainerTableName);
...
#region *** Функции контейнера *** - реализованы
        ...
        public override void ContainerLoad(Mary.MEngine cont)
        {
            ...
            string query = String.Format("SELECT {0}.* FROM {0};", MsJetDbAdapter.ContainerTableName);
        ...
        {
            string query = "UPDATE " + MsJetDbAdapter.ContainerTableName + " SET version = ?, step = ?, lognum = ?, loglevel = ?, descr = ?, name = ?, sflag = ?, state = ?, cellmode = ?, idcon = ? ;";
        ...
        private void insertEngineTable(MEngine cont)
        {
            ...
            string query = "INSERT INTO "+ MsJetDbAdapter.ContainerTableName + " (version, step, lognum, loglevel, descr, name, sflag, state, cellmode, idcon) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
...
#region *** Функции ячеек *** - реализованы
        ...
        public override bool isCellExists(Mary.MID cellid)
        {
            ...
            {
                string query = String.Format("SELECT {0}.cellid FROM {0} WHERE (cellid = ?);", MsJetDbAdapter.CellTableName);
        ...
        {
            String query = String.Format("SELECT COUNT(id) FROM {0} WHERE (name = {1})", MsJetDbAdapter.CellTableName, cellName);
        ...
        {
            return this.getRowCount(MsJetDbAdapter.CellTableName, "id");
        }
        /// <summary>
        ...
        {
            int id = this.getTableMaxId(MsJetDbAdapter.CellTableName, "cellid");
            if (id < 0) id = 0;
            return id;
        }
        /// <summary>
        ...
        {
            int id = this.getTableMinId(MsJetDbAdapter.CellTableName, "cellid");
            if (id < 0) id = 0;
            return id;
        }
        ...
        public override int CellInsert(Mary.MCellB cell)
        {
            ...
            {
                string query = String.Format("INSERT INTO {0} (name, descr, active, type, creatime, moditime, ronly, state, sflag, val, valtype, cellid) VALUES (?,?,?,?,?,?,?,?,?,?,?,?);", MsJetDbAdapter.CellTableName);
        ...
        public override Mary.MCell CellSelect(Mary.MID cellId, bool largeCell)
        {
            ...
            {
                string query = String.Format("SELECT {0}.* FROM {0} WHERE (cellid = ?);", MsJetDbAdapter.CellTableName); 
            ...
            if (rdr.HasRows == true)
            {
                if (largeCell == false)
                {
                    r = new MCellA(cellId);//create small cell
                }
                else
                {
                    rdr.Read();
        ...
        public override int CellUpdate(Mary.MCellB cell)
        {
            ...
            {
                string query = String.Format("UPDATE {0} SET name = ?, descr = ?, active = ?, type = ?, creatime = ?, moditime = ?, ronly = ?, state = ?, sflag = ?, val = ?, valtype = ? WHERE (cellid = ?);", MsJetDbAdapter.CellTableName);
        ...
        public override List<Mary.MCell> getBlockOfCells(int rowFrom, int rowTo)
        {
            ...
            {
                string query = String.Format("SELECT {0}.* FROM {0} WHERE ((cellid >= ?) AND (cellid < ?))", MsJetDbAdapter.LinkTableName);
        ...
        public override Mary.MLinkCollection getCellLinks(int cellid, Mary.MAxisDirection axisDir)
        {
            ...
            //set query string
            String query = String.Format("SELECT {0}.* FROM {0} WHERE ", MsJetDbAdapter.LinkTableName);
            String wherepart = "";
            switch (axisDir)
            {
        ...
        public override Mary.MCellCollection getCellsByTemplate(Mary.MCellTemplate tmp, bool largeCells)
        {
            ...
            sb.AppendFormat("SELECT {0}.{1} FROM {0} WHERE ", MsJetDbAdapter.CellTableName, ss);
        ...
        private OleDbDataReader getCellColumnDataReader(string columnName, int cellid)
        {
            ...
            //execute command
            string query = String.Format("SELECT {0}.{1} FROM {0} WHERE (cellid = ?);", MsJetDbAdapter.CellTableName, columnName);
        ...
        private void setCellColumnData(string columnName, int cellid, OleDbType dbtype, Object val)
        {
            ...
            //execute command
            string query = String.Format("UPDATE {0} SET {1} = ?, moditime = ? WHERE (cellid = ?);", MsJetDbAdapter.CellTableName, columnName);
        ...
        #region *** Функции связей *** - одна не реализована
        ...
        public override int LinkDelete(int linkId)
        {
            ...
            {
                string query = "DELETE FROM " + MsJetDbAdapter.LinkTableName + " WHERE (id = ?)"; 
        ...
        {
            string query = "INSERT INTO " + MsJetDbAdapter.LinkTableName + " (downID, upID, axis, state, active, sflag, descr, moditime) VALUES  (?,?,?,?,?,?,?,?);";
        ...
        {
            string query = "UPDATE " + MsJetDbAdapter.LinkTableName + " SET downID = ?, upID = ?, axis = ?, state = ?, active = ?, sflag = ?, descr = ?, moditime = ? WHERE (id = ?)";
        ...
        public override Mary.MLinkCollection getBlockOfLinks(int rowFrom, int rowTo)
        {
            ...
            {
                string query = String.Format("SELECT {0}.* FROM {0} WHERE ((id >= ?) AND (id < ?));", MsJetDbAdapter.LinkTableName);
        ...
        public override int getLinkID(Mary.MID dnCellId, Mary.MID upCellId, Mary.MID axis)
        {
            ...
            {
                string query = String.Format("SELECT {0}.id FROM {0} WHERE (downID = ?) AND (upID = ?) AND (axis = ?)", MsJetDbAdapter.LinkTableName);
        ...
        public override Mary.MLinkCollection getLinks(Mary.MLinkTemplate tmp)
        {
            ...
            OleDbCommand alias = new OleDbCommand("", (OleDbConnection)m_connection, (OleDbTransaction)m_transaction);
            //Создаем текст запроса и параметры
            sb.AppendFormat("SELECT {0}.* FROM {0} WHERE ", MsJetDbAdapter.LinkTableName);
        ...
        {
            return this.getTableMaxId(MsJetDbAdapter.LinkTableName, "id");
        }
        /// <summary>
        ...
        {
            return this.getTableMinId(MsJetDbAdapter.LinkTableName, "id");
        }
        /// <summary>
        ...
        {
            return this.getRowCount(MsJetDbAdapter.LinkTableName, "id");
        }
 #endregion
